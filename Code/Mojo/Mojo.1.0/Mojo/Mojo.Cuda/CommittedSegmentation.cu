...
struct ReplaceNeuralProcessInCommittedSegmentationFunction
{
    enum TupleLayout
    {
        ID_MAP,
    ...
    ReplaceNeuralProcessInCommittedSegmentationFunction( int oldId, int newId, uchar4 newColor ) : mOldId( oldId ), mNewId( newId ), mNewColor( newColor ) {};
...
extern "C" void ReplaceNeuralProcessInCommittedSegmentation2D( Mojo::Core::SegmenterState* segmenterState, int oldId, int newId, uchar4 newColor, int slice )
{
    ...
    MOJO_THRUST_SAFE(
        thrust::for_each(
            ...
            ReplaceNeuralProcessInCommittedSegmentationFunction( oldId, newId, newColor ) ) );
}

extern "C" void ReplaceNeuralProcessInCommittedSegmentation3D( Mojo::Core::SegmenterState* segmenterState, int oldId, int newId, uchar4 newColor )
{
    MOJO_THRUST_SAFE(
        thrust::for_each(
            thrust::make_zip_iterator(
...
extern "C" void InitializeSegmentationAndRemoveFromCommittedSegmentation( Mojo::Core::SegmenterState* segmenterState, int neuralProcessId )
{
    MOJO_THRUST_SAFE(
        thrust::for_each(
            thrust::make_zip_iterator(
            ...
            InitializeSegmentationAndRemoveFromCommittedSegmentationFunction(
                ...
                segmenterState->parameters.Get< float  >( "PRIMAL_MAP_BACKGROUND"   ) ) ) );
}
