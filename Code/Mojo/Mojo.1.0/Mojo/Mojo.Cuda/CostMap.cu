    ...
    if ( segmenterState->slicesWithForegroundConstraints.size() > 1 )
    {
        ...
        MOJO_FOR_EACH( int sliceWithConstraints, slicesWithConstraints )
        {
            if ( sliceWithConstraints >= minSliceWithForegroundConstraints &&
                 ...
                 !segmenterState->parameters.Get< bool >( "DIRECT_SCRIBBLE_PROPAGATION" ) )
            {
                for ( int i = 0; i < segmenterState->parameters.Get< float >( "PRIMAL_MAP_ERODE_NUM_PASSES" ); i++ )
                {
                    Mojo::Core::Thrust::Memcpy2DToArray( tempScratchpadMapArray, segmenterState->deviceVectors.Get< float >( "ScratchpadMap" )[ numElementsPerSlice * sliceWithConstraints ], segmenterState->volumeDescription );
            ...
            MOJO_THRUST_SAFE(
                thrust::for_each(
                    ...
                    InitializeCostMapOnSliceWithForegroundConstraintsFunction(
                        ...
                        segmenterState->parameters.Get< float >( "COST_MAP_MAX_VALUE" ) ) ) );
        ...
        for ( int z = minSliceWithForegroundConstraints + 1; z < maxSliceWithForegroundConstraints; z++ )
        {
            if ( std::find( slicesWithConstraints.begin(), slicesWithConstraints.end(), z ) == slicesWithConstraints.end() )
            {
                ...
                IncrementCostKernel< TCudaType ><<< gridDimensions, blockDimensions >>>(
                    ...
                    z,
                    -1,
                    segmenterState->parameters.Get< float >( "COST_MAP_OPTICAL_FLOW_IMPORTANCE_FACTOR" ),
                    segmenterState->parameters.Get< float >( "COST_MAP_MAX_VALUE" ) );
...
extern "C" void FinalizeCostMapFromPrimalMap( Mojo::Core::SegmenterState* segmenterState )
{
    if ( segmenterState->slicesWithForegroundConstraints.size() > 1 )
    {
        ...
        segmenterState->minCostsPerSlice.clear();
        ...
        for ( int z = minSliceWithForegroundConstraints; z <= maxSliceWithForegroundConstraints; z++ )
        {
            ...
            segmenterState->minCostsPerSlice[ z ] = minCostOnSlice;
        }
            ...
            MOJO_THRUST_SAFE(
                thrust::for_each(
                    ...
                    UpdateConstraintMapAndPrimalMapFromCostMapFunction(
                        segmenterState->minCostsPerSlice[ z ],
                        segmenterState->minCostsPerSlice[ z ] + segmenterState->maxForegroundCostDelta,
                        segmenterState->parameters.Get< float >( "CONSTRAINT_MAP_INITIALIZE_FROM_COST_MAP_MIN_FOREGROUND" ),
                        segmenterState->parameters.Get< float >( "CONSTRAINT_MAP_INITIALIZE_FROM_COST_MAP_MAX_BACKGROUND" ),
                        ...
                        segmenterState->parameters.Get< float >( "PRIMAL_MAP_FOREGROUND" ) ) ) );
        }
    }
}
