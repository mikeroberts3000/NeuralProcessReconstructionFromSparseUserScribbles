...
template < typename TCudaType >
void InitializeEdgeXYMapInternal( Mojo::Core::SegmenterState* segmenterState, Mojo::Core::Dictionary< Mojo::Core::VolumeDescription >* volumeDescriptions, float minSourceValue, float maxSourceValue )
{
    ...
    MOJO_THRUST_SAFE(
        thrust::for_each(
            ...
            InitializeEdgeXYMapFunction< TCudaType >(
                ...
                minSourceValue,
                maxSourceValue,
                segmenterState->parameters.Get< float >( "EDGE_MULTIPLIER" ),
                segmenterState->parameters.Get< float >( "EDGE_POWER_XY" ),
                segmenterState->parameters.Get< float >( "EDGE_MAX_BEFORE_SATURATE" ) ) ) );
}

extern "C" void InitializeEdgeXYMap( Mojo::Core::SegmenterState* segmenterState, Mojo::Core::Dictionary< Mojo::Core::VolumeDescription >* volumeDescriptions )
{
...
template < typename TCudaType >
void InitializeEdgeXYMapForSplittingInternal( Mojo::Core::SegmenterState* segmenterState, Mojo::Core::Dictionary< Mojo::Core::VolumeDescription >* volumeDescriptions, int id, float minSourceValue, float maxSourceValue )
{
    int numElements = volumeDescriptions->Get( "FilteredSourceMap" ).numVoxels.x * volumeDescriptions->Get( "FilteredSourceMap" ).numVoxels.y * volumeDescriptions->Get( "FilteredSourceMap" ).numVoxels.z;
    ...
    MOJO_THRUST_SAFE(
        Mojo::Core::Thrust::Fill(
            ...
            segmenterState->parameters.Get< float >( "STENCIL_MAP_INITIAL_VALUE" ),
    ...
    MOJO_THRUST_SAFE(
        thrust::for_each(
            ...
            InitializeStencilMapFromIdMapFunction(
                id,
                segmenterState->parameters.Get< float >( "STENCIL_MAP_BACKGROUND_VALUE" ) ) ) );


    //
    // shrink the background regions slightly
    //
    for ( int i = 0; i < segmenterState->parameters.Get< int >( "CONSTRAINT_MAP_SPLIT_BACKGROUND_CONTRACT_NUM_PASSES" ); i++ )
    {
        MOJO_THRUST_SAFE(
            thrust::for_each(
                thrust::make_transform_iterator(
    ...
    MOJO_THRUST_SAFE(
        thrust::for_each(
            ...
            UpdateStencilMapFunction(
                ...
                segmenterState->parameters.Get< float >( "STENCIL_MAP_INITIAL_VALUE" ),
                segmenterState->parameters.Get< float >( "STENCIL_MAP_BACKGROUND_VALUE" ),
                segmenterState->parameters.Get< float >( "STENCIL_MAP_STRONGEST_EDGE_VALUE" ) ) ) );
    ...
    MOJO_THRUST_SAFE(
        thrust::for_each(
            ...
            InitializeEdgeXYMapForSplittingFunction< TCudaType >(
                ...
                segmenterState->parameters.Get< float >( "EDGE_MULTIPLIER" ),
                segmenterState->parameters.Get< float >( "EDGE_POWER_XY" ),
                segmenterState->parameters.Get< float >( "EDGE_SPLIT_BOOST" ),
                segmenterState->parameters.Get< float >( "STENCIL_MAP_STRONGEST_EDGE_VALUE" ),
                segmenterState->parameters.Get< float >( "STENCIL_MAP_WEAKEST_EDGE_VALUE" ) ) ) );
}

extern "C" void InitializeEdgeXYMapForSplitting( Mojo::Core::SegmenterState* segmenterState, Mojo::Core::Dictionary< Mojo::Core::VolumeDescription >* volumeDescriptions, int id )
{
    switch ( segmenterState->volumeDescription.dxgiFormat )
    {
        case DXGI_FORMAT_R8_UNORM:
